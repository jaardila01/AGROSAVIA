...
def ThreadActualizarSocket():
	...
	while True:
		...
		file1 = open("./resultados/datosCrearModelo/completos" + str(TIEMPO_INICIO) + ".txt", "a")
		try:
			...
			print(' ')
			nuevaLineaDatos = [ Ax,Ay,Az,Gx,Gy,Gz,IdClient,Ch4,NumeroPaquete, ACTIVIDAD_ACTUAL[IdClient], fechaYhora]
			...
			file1.write((fechaYhora + "	 %.1f	 %.5f	 %.5f	 %.5f	 %.5f	 %.5f	 %.5f	 %.5f" % (NumeroPaquete, IdClient, Ax, Ay, Az, Gx, Gy, Gz)) + "	" + str(ACTIVIDAD_ACTUAL[IdClient]) + "	\n")
			file1.close()
